...
function fazerLogout() {
    localStorage.removeItem('softrh_autenticado');
    localStorage.removeItem('softrh_login_time');
    ...
    setupEventListeners() {
        ...
        if (userMenu) {
            ...
            const alternarMenuUsuario = () => {
                const aberto = userMenu.classList.toggle('is-open');
                userMenu.setAttribute('aria-expanded', String(aberto));
            };
            ...
            document.addEventListener('click', (event) => {
                if (!userMenu.contains(event.target)) fecharMenuUsuario();
            });
        }
