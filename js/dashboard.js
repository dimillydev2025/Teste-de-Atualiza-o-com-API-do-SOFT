...
function createOrUpdateChart(ctxId, config){
  try{
    ...
    if(!ctx) return null;
    if(window._softRhCharts[ctxId]) {
      try{ window._softRhCharts[ctxId].destroy(); } catch(e){ console.warn('chart destroy error', e); }
      window._softRhCharts[ctxId] = null;
    }
    const c = new Chart(ctx, config);
    window._softRhCharts[ctxId] = c;
    return c;
  }catch(e){
